...
"""
广告插入模块 - 重构版本
在视频指定位置和时间插入小比例广告视频，保持原视频时长，智能音频切换
"""
...
class AdInserter(object):
    ...
    def __init__(self, config=None):
        ...
        self.ffmpeg_path = 'ffmpeg'  # 使用系统PATH中的ffmpeg
    ...
    def _build_audio_filter(self, main_has_audio, ad_has_audio, start_time, duration, total_duration):
        ...
        if not main_has_audio and ad_has_audio:
            # 只有广告有音频 - 在广告时间段播放广告音频，其余时间静音
            return (
                "[1:a]atrim=0:{},asetpts=PTS-STARTPTS+{}/TB,apad=pad_dur={}[audio_out]"
            ).format(duration, start_time, total_duration)
        
        if main_has_audio and not ad_has_audio:
            # 只有主视频有音频 - 直接使用主视频音频，限制时长
            return "[0:a]atrim=0:{}[audio_out]".format(total_duration)
        
        # 都有音频 - 智能切换：广告时间用广告音频，其他时间用主视频音频
        ...
        if start_time > 0:
            ...
            if end_time < total_duration:
                # 前置主音频 + 广告音频 + 后置主音频
                return (
                    "[0:a]atrim=0:{},asetpts=PTS-STARTPTS[main_pre];"
                    "[1:a]atrim=0:{},asetpts=PTS-STARTPTS[ad_audio];"
                    "[0:a]atrim={}:{},asetpts=PTS-STARTPTS[main_post];"
                ...
                ).format(start_time, total_duration - start_time)
        else:
            # 从开头就是广告
            if end_time < total_duration:
                # 广告音频 + 后置主音频
                ...
                # 全程广告音频
                return (
                    "[1:a]atrim=0:{},apad=pad_dur={}[audio_out]"
                ).format(total_duration, total_duration)
    
    def calculate_ad_dimensions(self, main_video_info, ad_video_info, ad_scale=0.25):
        """计算广告视频的尺寸和位置，保持广告原始比例"""
        main_width = main_video_info['width']
        main_height = main_video_info['height']
        
        # 获取广告的原始比例
        ad_original_width = ad_video_info['width']
        ad_original_height = ad_video_info['height']
        ad_aspect_ratio = float(ad_original_width) / float(ad_original_height)
        
        # 按比例缩放，但保持原始长宽比
        # 先按主视频的某一边进行缩放计算
        target_width = int(main_width * ad_scale)
        target_height = int(target_width / ad_aspect_ratio)
        
        # 检查高度是否超出主视频，如果超出则按高度重新计算
        max_height = int(main_height * ad_scale)
        if target_height > max_height:
            target_height = max_height
            target_width = int(target_height * ad_aspect_ratio)
        
        print(u"广告原始尺寸: {}x{} (比例: {:.2f})".format(
            ad_original_width, ad_original_height, ad_aspect_ratio
        ))
        print(u"缩放后尺寸: {}x{} (保持原比例)".format(target_width, target_height))
        
        # 广告位置（右上角，留出边距）
        margin = 20
        ad_x = main_width - target_width - margin
        ad_y = margin
    ...
    def insert_ad_overlay(self, main_video_path, ad_video_path, output_path, 
        ...
        - start_time: 广告开始时间（秒）
        - duration: 广告显示时长（秒）
        - position: 广告位置 ('top-right', 'top-left', 'bottom-right', 'bottom-left', 'center')
        ...
        # 获取广告视频信息（为了保持原始比例）
        ad_info = self.get_video_info(ad_video_path)
        if not ad_info:
            return False, u"无法获取广告视频信息"
        ...
        # 计算广告位置和尺寸（保持原始比例）
        ad_dims = self.calculate_ad_dimensions(main_info, ad_info, scale)
        ...
        if position == 'top-left':
            ...
            ad_dims['x'] = (main_info['width'] - ad_dims['width']) / 2
            ad_dims['y'] = (main_info['height'] - ad_dims['height']) / 2
        # top-right 已经是默认位置
        
        print(u"广告尺寸: {}x{} 位置: ({}, {})".format(
            ad_dims['width'], ad_dims['height'], ad_dims['x'], ad_dims['y']
        ))
        print(u"保持原视频时长: {:.1f}秒".format(main_info['duration']))
        ...
        # 构建视频滤镜 - 保持广告原始比例
        video_filter = (
            "[1:v]scale={}:{}:force_original_aspect_ratio=decrease,setpts=PTS-STARTPTS+{}/TB[ad_scaled];"
            "[0:v][ad_scaled]overlay={}:{}:enable='between(t,{},{})':shortest=1[video_out]"
        ).format(
            ad_dims['width'], ad_dims['height'],  # 目标缩放尺寸
            start_time,                           # 广告开始时间偏移
    ...
    def insert_multiple_ads(self, main_video_path, ad_configs, output_path):
        """
        插入多个广告 - 保持原视频时长，智能音频切换
        
        参数:
        - main_video_path: 主视频路径
        - ad_configs: 广告配置列表 [{'path': '', 'start': 0, 'duration': 5, 'position': '', 'scale': 0.25}, ...]
        - output_path: 输出路径
        """
        if not ad_configs:
            return False, u"没有广告配置"
        
        print(u"准备插入 {} 个广告，保持原视频时长".format(len(ad_configs)))
        ...
        print(u"主视频时长: {:.1f}秒".format(main_info['duration']))
        ...
        for i, config in enumerate(ad_configs):
            ...
            # 获取当前广告的视频信息以保持原始比例
            ad_info = self.get_video_info(config['path'])
            if not ad_info:
                return False, u"无法获取广告 {} 的视频信息".format(i+1)
            ...
            # 缩放广告（保持原始比例）
            scale_filter = "[{}:v]scale={}:{}:force_original_aspect_ratio=decrease,setpts=PTS-STARTPTS+{}/TB[ad{}_scaled]".format(
                ad_index, ad_dims['width'], ad_dims['height'], start_time, i
            )
            filter_parts.append(scale_filter)
            
            # 叠加
            overlay_input = overlay_inputs[-1]
            overlay_filter = "{}[ad{}_scaled]overlay={}:{}:enable='between(t,{},{})':shortest=1[overlay{}]".format(
        ...
        # 音频处理 - 简化版本，只使用主视频音频（多广告音频混合复杂）
        if main_has_audio:
            audio_filter = "[0:a]atrim=0:{}[audio_out]".format(main_info['duration'])
            filter_complex = video_filter + ";" + audio_filter
        else:
            filter_complex = video_filter
        
        cmd.extend(['-filter_complex', filter_complex])
        cmd.extend(['-map', overlay_inputs[-1]])  # 最终视频输出
